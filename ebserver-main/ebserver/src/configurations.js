...
function plainConfigs(config) {
    const plainConfig = {}
    for (const key of config["program-types"]) {
...
function loadConfigs(){
    const folderPath = path.join(__dirname, "configurationsFolder");
    const configFiles = fs.readdirSync(folderPath);
    configFiles.forEach((configuration) => {
        const raw = fs.readFileSync(path.join(folderPath, configuration));
