...
import pt.ulisboa.ciencias.userenergy.enums.AirplaneMode;
import pt.ulisboa.ciencias.userenergy.enums.BatterySaving;
import pt.ulisboa.ciencias.userenergy.enums.Bright;
import pt.ulisboa.ciencias.userenergy.enums.Refresh;
import pt.ulisboa.ciencias.userenergy.enums.Theme;
...
public class Util {
    ...
    /**
     * Test: Set screen brightness to two-thirds (~67%).
     */
    @Test
    public void brightness_TwoThirds() throws IOException, InterruptedException {
        setBrightness(Bright.TWOTHIRDS);
    }
    ...
    public void resetDefaultValues() {
        ...
        try {
            for (String[] entry : entries) {
                String result = device.executeShellCommand(
                        "settings delete " + entry[0] + " " + entry[1]).trim();
                if (result.toLowerCase().contains("error")) {
                    failures.append(entry[1]).append(": ").append(result).append("\n");
                }
            }
        } catch (IOException e) {
        ...
        if (failures.length() > 0) {
            throw new AssertionError("Some settings failed to reset:\n" + failures);
        }
    }
}
